...
var _results = _interopRequireDefault(require("./results.js"));
...
function tabulate(nbrSeatsToFill, candidates, ballots, maxRankingLevels, tieBreaker, excluded, protectedzz, options = {}, progressCallback = null) {
...
class NodeStats {
  ...
  toString() {
    const result = String(this.label) + ': nodes=' + this.nbrNodes + ' leaves=' + this.nbrLeaves + ' visited=' + this.nbrVisited + ' rounds=' + this.nbrRounds + ' iterations=' + this.nbrIterations + ' added=' + this.nbrAdded + ' deleted=' + this.nbrDeleted + ' elapsed=' + this.elapsedTime.toFixed(3);
    return result;
  }
...
class Tabulation {
   ...
   *
   */
  constructor(nbrSeatsToFill, candidates, ballots, maxRankingLevels, tieBreaker, excluded = [], protectedzz = [], options = {}, progressCallback) {
    try {
      ...
      if (progressCallback instanceof _progress.Progress) {
        ...
        this._progress = new _progress.Progress(progressCallback);

        this._progress.setNbrBallotGroups(Array.isArray(this._ballots) ? this._ballots.length : 10);
      }

      const validator = new _validate.Validator();
      ...
      if ([1, 2, 3].indexOf(TYPETREE) >= 0) {
        this._options[_constants.default.OPTIONS.ballotTree._value] = [null, _constants.default.OPTIONS.ballotTree.none, _constants.default.OPTIONS.ballotTree.static, _constants.default.OPTIONS.ballotTree.dynamic][TYPETREE];
      }

      Object.assign(this._options, optionsValidated);

      if (this._options[_constants.default.OPTIONS.ballotTree._value] === _constants.default.OPTIONS.ballotTree.static) {
        this._progress.useBallotTree();
      }

      if (this._options[_constants.default.OPTIONS.ballotTree._value] === _constants.default.OPTIONS.ballotTree.dynamic) {
      ...
      console.debug(_util_basic.UBF.describeError(exc));

      if (exc instanceof _errors.MeekValueError || exc instanceof _errors.MeekImplementationError) {
        throw exc;
      }

      console.error(_util_basic.UBF.describeError(exc));

      if (exc instanceof RangeError || exc instanceof ReferenceError || exc instanceof SyntaxError || exc instanceof TypeError) {
        throw new _errors.MeekImplementationError('Likely Meek implementation error:', [], exc);
      }
  ...
  _otherCategories() {
    ...
    _constants.default.OTHER_LABELS_LIST.forEach(otherLabel => {
      if (this._protectedzz.size === 0 && (otherLabel === _constants.default.LABEL.protectedQuota || otherLabel === _constants.default.LABEL.quotaVotes)) {
        return;
      }
  ...
  _tabulateMeeks() {
    ...
    */
    ...
    return new _results.default(this._elected(), this._statuses, this._tallies);
  }
  /* */
  ...
  _tabulateSetup() {
    ...
    */
    ...
    this._candidates.forEach(candidate => {
      this._statuses[candidate] = new _status.Status(candidate, null, this._nbrRound, undefined, undefined, this._getCandidateDestiny(candidate));
    });

    for (let cstatus in this._statuses) {
      if (this._excluded.has(cstatus)) {
        this._statuses[cstatus].status = _constants.default.STATUS.defeated;
      }
    }

    this._omega = _constants.default.ULP.times(1000);
    this._keepFactors = {};

    this._candidates.forEach(candidate => {
      this._keepFactors[candidate] = _constants.default.ONE;
    });
    ...
    } else if (this._options[_constants.default.OPTIONS.ballotTree._value] === _constants.default.OPTIONS.ballotTree.dynamic) {
  ...
  _subTreeToString(node, prefix = '', isLastChild = false) {
    ...
    childKeys.forEach((child, ix) => {
      const newPrefix = prefix === '' ? ' + ' : prefix.slice(0, -2) + (isLastChild ? ' ' : '|') + '  + ';
      lines += this._subTreeToString(node.children[child], newPrefix, ix === childKeys.length - 1);
    });
    return lines;
  }
  ...
  _buildDynamicBallotTree() {
    ...
    this._hopefulParents = Array.from(this._hopeful()).reduce((result, hopeful) => {
  ...
  _distributeBallotGroup(ballotGroup, parentNode) {
    ...
    for (ballotGroup.lastUsedIndex++; ballotGroup.lastUsedIndex < rankings.length; ballotGroup.lastUsedIndex++) {
      ...
      if (rankingCode === _constants.default.RANKING_CODE.undervote || ballotGroup.wellRanked.has(rankingCode)) {
        continue;
      }
      ...
      if (this._statuses[rankingCode] && this._statuses[rankingCode].status === _constants.default.STATUS.defeated) {
        continue;
      }

      const tabCat = rankingCode === _constants.default.RANKING_CODE.overvote ? _constants.default.LABEL.overvotes : rankingCode;

      const childNode = this._getOrMakeChild(tabCat, parentNode);

      childNode.ballotCount += multiple;

      if (tabCat === _constants.default.LABEL.overvotes) {
        ballotGroup.lastUsedIndex = rankings.length + 1;
        break;
      }

      if (this._statuses[rankingCode].status === _constants.default.STATUS.hopeful) {
    ...
    if (ballotGroup.lastUsedIndex === rankings.length) {
      const exhaustedLabel = ballotGroup.wellRanked.size < this._maxWellRanked ? _constants.default.LABEL.abstentions : _constants.default.LABEL.otherExhausted;
  ...
  _getOrMakeChild(name, parentNode) {
    ...
    if (!childNode) {
      ...
      if ('ballotGroups' in parentNode) {
        ...
        if (name in this._statuses && this._statuses[name].status === _constants.default.STATUS.hopeful) {
  ...
  _buildBallotTree() {
    ...
    this._ballots.forEach((ballot, ix) => {
      ...
      const hadNoOvervote = rankings.every((rankingCode, ix) => {
        if (rankingCode === _constants.default.RANKING_CODE.undervote || wellRanked.has(rankingCode)) {
          return true;
        }

        if (rankingCode !== _constants.default.RANKING_CODE.overvote) {
          nbrWellRanked++;
        }
      ...
      if (hadNoOvervote && (depth < nbrHopeful - 1 || nbrHopeful === 0)) {
        const exhaustedLabel = nbrWellRanked < this._maxWellRanked ? _constants.default.LABEL.abstentions : _constants.default.LABEL.otherExhausted;
  ...
  _processAMeeksRound() {
    ...
    */
    if (this._nbrRound > 0 || this._nbrRound === 0 && this._options[_constants.default.OPTIONS.alwaysCountVotes._value] === _constants.default.OPTIONS.alwaysCountVotes.no) {
      // Reference rule step B.1, Test count complete
      const nbrElectedOrProtected = _util_basic.UBF.setUnion(this._elected(), this._protectedzz).size;
      ...
      if (nbrElectedOrProtected === this._nbrSeatsToFill || this._hopeful().size + this._elected().size <= this._nbrSeatsToFill) {
        ...
        if (nbrElectedOrProtected < this._nbrSeatsToFill) {
          ...
          this._electCandidates(_util_basic.UBF.setDifference(this._protectedzz, this._elected()));
    ...
    if (this._nbrRound > this._candidates.length + 2) {
      throw new _errors.MeekImplementationError('Error: Tabulation requires too many rounds', [['starting round #', this._nbrRound], ['nbr of candidates', this._candidates.length]]);
      return false;
    }
    ...
    for (let label in this._iterTally) {
      ...
      if (this._statuses[label] === undefined || this._statuses[label].status !== _constants.default.STATUS.defeated) {
    ...
    if (!this._newElecteds.size) {
      ...
      if (this._getAltDefeatsOption() === _constants.default.OPTIONS.alternativeDefeats.yes && this._options[_constants.default.OPTIONS.typeOfAltDefs._value] === _constants.default.OPTIONS.typeOfAltDefs.beforeSingleDefeats) {
  ...
  _processAMeeksIteration() {
    ...
    this._iterTally[_constants.default.LABEL.totalCandidateVotes] = this._totalCandidateVotes;
    this._iterTally[_constants.default.LABEL.totalVotes] = this._iterTally[_constants.default.LABEL.totalCandidateVotes].plus(this._iterTally[_constants.default.LABEL.overvotes]).plus(this._iterTally[_constants.default.LABEL.abstentions]).plus(this._iterTally[_constants.default.LABEL.otherExhausted]); //# Reference rule step B.2.b, Update quota
    ...
    this._iterTally[_constants.default.LABEL.totalSurplus] = this._totalSurplus; // Reference rule step B.2.e, Test for iteration finished
    ...
    if (this._getAltDefeatsOption() === _constants.default.OPTIONS.alternativeDefeats.yes && this._options[_constants.default.OPTIONS.typeOfAltDefs._value] === _constants.default.OPTIONS.typeOfAltDefs.ifNoNewElecteds) {
      this._defeatThisRound = this._getStvAlternativeDefeats();
    ...
    if (this._getAltDefeatsOption() === _constants.default.OPTIONS.alternativeDefeats.yes && this._options[_constants.default.OPTIONS.typeOfAltDefs._value] === _constants.default.OPTIONS.typeOfAltDefs.perRefRule) {
      this._defeatThisRound = this._getStvAlternativeDefeats();
  ...
  _initIterTally() {
    ...
    for (let tabCode in this._tallies) {
      if (this._statuses[tabCode] !== undefined && this._statuses[tabCode].status === _constants.default.STATUS.defeated) {
        this._iterTally[tabCode] = null;
      } else {
        this._iterTally[tabCode] = _constants.default.ZERO;
      }
    }

    this._iterTally[_constants.default.LABEL.nbrIterations] = this._nbrIteration;
  }
  ...
  _distributeVotesByTree() {
    ...
    */
    this._distributeFromNode(this._ballotTree, _constants.default.ONE, 0);
  }
  ...
  _distributeFromNode(node, ballotWeight, depth) {
    ...
    if (node.name !== ':root' && (!this._statuses[rankingCode] || this._statuses[rankingCode].status !== _constants.default.STATUS.defeated)) {
      let rankingKeep = ballotWeight;

      if (this._statuses[rankingCode] !== undefined) {
        if (this._statuses[rankingCode].status === _constants.default.STATUS.hopeful || this._statuses[rankingCode].status === _constants.default.STATUS.elected) {
          rankingKeep = ballotWeight.times(this._keepFactors[rankingCode], _constants.default.ROUND.away);
        }
      }
      ...
      try {
        ...
        throw new _errors.MeekImplementationError('Error: Failure to accumulate votes to iteration tally.', [['node name', node.name], ['node ballotCount', node.ballotCount], ['rankingCode', rankingCode], ['iterTally[rankingCode]', JSON.stringify(this._iterTally[rankingCode])]], exc);
      }

      ballotWeight = ballotWeight.minus(rankingKeep);
    }

    if (ballotWeight.isEqual(_constants.default.ZERO)) {
      return;
    }
  ...
  _distributeVotes() {
    ...
    */
    this._ballots.forEach(ballot => {
      let ballotWeight = _constants.default.ONE;
      const wellRanked = new Set();
      const multiple = ballot.getMultiple();
      ballot.getRankings().every((rankingCode, rankingIndex) => {
        if (rankingCode === _constants.default.RANKING_CODE.overvote) {
          this._iterTally[_constants.default.LABEL.overvotes] = this._iterTally[_constants.default.LABEL.overvotes].plus(ballotWeight.times(multiple));
          ballotWeight = _constants.default.ZERO;
          this._currentNodeStats.nbrVisited++;
          return false;
        }

        if (this._statuses[rankingCode] !== undefined) {
          if ((this._statuses[rankingCode].status === _constants.default.STATUS.hopeful || this._statuses[rankingCode].status === _constants.default.STATUS.elected) && !wellRanked.has(rankingCode)) {
            const rankingKeep = ballotWeight.times(this._keepFactors[rankingCode], _constants.default.ROUND.away);
            this._iterTally[rankingCode] = this._iterTally[rankingCode].plus(rankingKeep.times(multiple));
            ballotWeight = ballotWeight.minus(rankingKeep);
            this._currentNodeStats.nbrVisited++;
      ...
      if (ballotWeight.isGreater(_constants.default.ZERO)) {
        this._currentNodeStats.nbrVisited++;
        const exhaustedVotes = ballotWeight.times(multiple);

        if (wellRanked.size < this._maxWellRanked) {
          this._iterTally[_constants.default.LABEL.abstentions] = this._iterTally[_constants.default.LABEL.abstentions].plus(exhaustedVotes);
        } else {
          this._iterTally[_constants.default.LABEL.otherExhausted] = this._iterTally[_constants.default.LABEL.otherExhausted].plus(exhaustedVotes);
        }
      }
    });
  }
  ...
  _getCandidateDestiny(candidate) {
    let result = _constants.default.DESTINY.normal;

    if (this._excluded.has(candidate)) {
      result = _constants.default.DESTINY.excluded;
    } else if (this._protectedzz.has(candidate)) {
      result = _constants.default.DESTINY.protected;
    }
  ...
  _getTotalCandidateVotes() {
    let result = _constants.default.ZERO;
  ...
  _setQuota() {
    ...
    quota = quota.plus(_constants.default.ULP);
    this._quota = quota;

    if (this._protectedzz.size) {
      this._protectedQuota = quota;
      this._iterTally[_constants.default.LABEL.protectedQuota] = this._protectedQuota;
      this._protectedVotes = _constants.default.ZERO;

      this._protectedzz.forEach(candidate => {
        this._protectedVotes = this._protectedVotes.plus(this._iterTally[candidate].isLess(this._protectedQuota) ? this._iterTally[candidate] : this._protectedQuota);
      });

      this._quotaVotes = this._totalCandidateVotes.minus(this._protectedVotes);
      this._iterTally[_constants.default.LABEL.quotaVotes] = this._quotaVotes;
      quota = this._quotaVotes.divideBy(this._nbrUnprotectedSeats + 1);
      this._quota = quota.plus(_constants.default.ULP);
    }

    this._iterTally[_constants.default.LABEL.quota] = this._quota;
  }

  _getQuota(candidate) {
    const result = this._protectedzz.has(candidate) ? this._protectedQuota : this._quota;
    return result;
  }
  ...
  _updateCandidateStatusTally() {
    for (let candidate in this._statuses) {
      ...
      if (candidateStatus.status === _constants.default.STATUS.hopeful) {
  ...
  _getTotalSurplus() {
    ...
    for (let candidate in this._statuses) {
      if (this._statuses[candidate].status === _constants.default.STATUS.elected) {
  ...
  _updateKeepFactors() {
    for (let candidate in this._keepFactors) {
      ...
      if (this._statuses[candidate].status === _constants.default.STATUS.elected) {
        ...
        const product = keepFactor.times(quota, _constants.default.ROUND.away);
        const newKeepFactor = product.divideBy(this._iterTally[candidate], _constants.default.ROUND.away);
  ...
  _hopeful() {
    ...
    */
    ...
    for (let candidate in this._statuses) {
      if (this._statuses[candidate].status === _constants.default.STATUS.hopeful) {
  ...
  _getNewElecteds() {
    ...
    */
    ...
    this._hopeful().forEach(candidate => {
      if (this._iterTally[candidate].isGreaterEqual(this._getQuota(candidate))) {
  ...
  _electCandidates(candidates) {
    ...
    */
    candidates.forEach(candidate => {
      if (this._statuses[candidate] instanceof _status.Status && this._statuses[candidate].status === _constants.default.STATUS.hopeful) {
        this._statuses[candidate].status = _constants.default.STATUS.elected;
      } else {
        throw new _errors.MeekImplementationError('Attempting to elect a candidate that is not hopeful.', [['candidate', candidate], ['status', this._statuses[candidate].status], ['round', this._nbrRound]]);
      }
    });
  }
  ...
  _defeatCandidates(candidates) {
    ...
    */
    candidates.forEach(candidate => {
      if (this._statuses[candidate] instanceof _status.Status && this._statuses[candidate].status === _constants.default.STATUS.hopeful && !this._protectedzz.has(candidate)) {
        this._statuses[candidate].status = _constants.default.STATUS.defeated;
        this._keepFactors[candidate] = _constants.default.ZERO;
      } else {
        throw (0, _errors.MeekImplementationError)('Attempting to defeat a candidate that is not defeatable.', [['candidate', candidate], ['status', this._statuses[candidate].status], ['round', this._nbrRound]]);
      }
    });
  }
  ...
  _getSingleDefeatCandidate() {
    ...
    let minVotes = _constants.default.ONE.times(_constants.default.Decimal.MAX_SAFE_VALUE + 1);
  ...
  _resolveTie(tiedCandidates) {
    ...
    const notInTieBreaker = _util_basic.UBF.setDifference(tiedCandidates, _util_basic.UBF.getKeys(this._tieBreaker));

    if (notInTieBreaker.size) {
      throw new _errors.MeekValueError('Tied candidate is not in tieBreaker:', [['candidate', Array.from(notInTieBreaker)[0]], ['round', this._nbrRound], ['tiedCandidates', tiedCandidates], ['tieBreaker', this._tieBreaker]]);
    }
  ...
  _getAltDefeatsOption() {
     ...
     */
    const optionsValue = this._options[_constants.default.OPTIONS.alternativeDefeats._value];
    ...
    if (typeof optionsValue === 'string') {
      ...
      if (optionsValue instanceof Array && this._nbrRound <= optionsValue.length) {
        ...
        throw new _errors.MeekValueError('Round by round value for alternative defeats option' + ' is too short:', [['nbrRound', this._nbrRound], ['optionsValue.length', optionsValue.length], ['optionsValue', optionsValue]]);
      }
    }
  ...
  _getStvAlternativeDefeats() {
    ...
    let totalVotesToDefeat = _util_basic.UBF.getOwnItems(defeatableVotes).reduce((sum, item) => {
      return sum.plus(item.value);
    }, _constants.default.ZERO);

    const byVotes = _util_basic.UBF.getOwnItems(defeatableVotes).sort((a, b) => {
      return a.value.isLess(b.value) ? 1 : a.value.isEqual(b.value) ? 0 : -1;
    });

    let candidatesToDefeat = new Set(_util_basic.UBF.getKeys(defeatableVotes));

    const nbrElected = this._elected().size; // Breaking out of this loop produces a set of candidates to defeat,
    ...
    byVotes.some((item, ix) => {
      ...
      const nbrRemaining = ix + 1; //nbrDefeated -= 1
      ...
      const cond_2 = totalVotesToDefeat.plus(totalSurplusVotes).isLess(lowestRemainingVotes);
      const cond_1 = nbrRemaining + nbrElected >= this._nbrSeatsToFill;
      const condAltDef = cond_1 && cond_2; // Test top-level conditions
