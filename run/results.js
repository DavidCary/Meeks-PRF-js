...
class Results {
  ...
  static getElectedAsString(elected, prefix = '', label = '"elected": ', indent1 = 2, suffix = '') {
    const electedArray = Array.from(elected);
    electedArray.sort();

    const coreContent = _util_basic.UBF.show(electedArray);
  ...
  static getStatusesAsString(statuses, prefix = '', label = '"status": ', indent1 = 2, indent2 = 4, suffix = '') {
    const statusItems = _util_basic.UBF.getOwnItems(statuses);
    ...
    statusItems.forEach((item, index) => {
      ...
      item.value.asArray().forEach((entry, ix) => {
        ...
        if (typeof entry === 'string') {
          line += _util_basic.UBF.show(entry);
        } else if (entry instanceof _constants.default.Decimal) {
  ...
  static getTallyAsString(tally, statuses, prefix = '', label = '"tally": ', indent1 = 2, indent2 = 4, suffix = '') {
    const tallyItems = _util_basic.UBF.getOwnItems(tally);
    ...
    tallyItems.forEach(item => {
      ...
      item.value.forEach((entry, ix) => {
        ...
        if (entry instanceof _constants.default.Decimal) {
          if (Number.isInteger(entry.toNumber())) {
  ...
  static getSortKey(code, statuses) {
    ...
    if (statuses[code] !== undefined) {
      ...
      if (votes === null) {
        votes = _constants.default.ONE.negative();
      }

      if (statuses[code].status === _constants.default.STATUS.elected) {
        sortKey = [1, 1, nbrRound, votes.negative(), code];
      } else if (statuses[code].status === _constants.default.STATUS.hopeful) {
        sortKey = [1, 2, -nbrRound, votes.negative(), code];
      } else {
        sortKey = [1, 3, -nbrRound, votes.negative(), code];
      }
    } else {
      let otherLabelsIndex = _constants.default.OTHER_LABELS_LIST.indexOf(code);
  ...
  static compareSortKeys(a, b) {
    ...
    a.sortKey.some((aKey, ix) => {
      ...
      if (aKey instanceof _constants.default.Decimal) {
        if (aKey.isLess(bKey)) {
          comparison = -1;
          return true;
        }
