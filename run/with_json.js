...
var Meek = _interopRequireWildcard(require("./meek.js"));
...
const D9 = _constants.default.Decimal;
...
class WithJson {
  ...
  static async tabulate(inputJson = '', outputJson = '', defaultJson = null) {
    const [tabulateArgs, tabulationSpec] = await WithJson.buildTabulateArgs(inputJson, defaultJson);
    ...
    const results = new Meek.Tabulation(...tabulateArgs).tabulate();
    const jsonStr = WithJson.resultsToJsonString(results, description);
  ...
  static async buildTabulateArgs(inputJson, defaultJson) {
    const tabulationSpec = await WithJson.readTabulationSpec(inputJson, defaultJson);
    let argBallots = tabulationSpec['ballots'];

    if ('ballotsMore' in tabulationSpec) {
      argBallots = Array.from(argBallots);
      argBallots = argBallots.concat(Array.from(tabulationSpec['ballotsMore']));
    }

    const tabulateArgs = [tabulationSpec['nbrSeatsToFill'], tabulationSpec['candidates'], argBallots, tabulationSpec['maxRankingLevels'], tabulationSpec['tieBreaker'], tabulationSpec['excluded'], tabulationSpec['protected'], tabulationSpec['options']];
  ...
  static async readFile(fileName) {
    ...
    if (fileName == '') {
      ...
      text = await readPromise; //await console.debug('readFile text set with await');
  ...
  static async readJson(fileName) {
    const text = await WithJson.readFile(fileName); //console.debug('readJson text='+Meek._show(text));
  ...
  static async readOptionalJson(fileName) {
    ...
    if (typeof fileName == 'string' && fileName != '' && fs.existsSync(fileName)) {
  ...
  static async readTabulationSpec(inputJson, defaultJson) {
    ...
    if (defaultJson !== null && defaultJson !== undefined && !(defaultJson == '' && inputJson == '')) {
    ...
    */
    ...
    const primarySpec = await WithJson.readJson(inputJson); //console.debug('primarySpec='+Meek._show(primarySpec));
    ...
    for (let iix in includeList) {
      const includeInputJson = includeList[iix]; //console.debug('iix='+iix+' includeInputJson='+includeInputJson);
    ...
    if (process && process.argv && Array.isArray(process.argv) && process.argv.length >= 2) {
      if (process.argv.length > 3) {
        ...
        if (process.argv[2] === '--version') {
          process.stdout.write('Version ' + _constants.default.VERSION_STRING + '\n');
        } else {
...
if (require && require.main && module && require.main === module && process && process.argv && Array.isArray(process.argv) && process.argv.length >= 2) {
